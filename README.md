...
- 제공자: OpenAI only
- 기본 리드 정책: `gpt-5.5`, medium reasoning
  - 일반 계획, 구현 조율, 보통 난도의 제품 작업에 기본으로 사용한다.
- 하위 worker 정책: `gpt-5.4`
  - 범위가 좁은 구현, 문서/evidence 정리, 긴 감사, 반복 요약처럼 저위험이거나 방향이 고정된 작업에 사용한다.
- 고성능 승격 정책: `gpt-5.6`, high reasoning
  - 초기 제품 방향, 아키텍처/데이터 경계, 인증/결제/보안, 복잡한 UX 계층, 큰 작업 분해, 실패 복구 계획, 계약/템플릿 정책 변경, 프롬프트/구조 감사, 최종 리뷰에 사용한다.
- UI checker 정책: `gpt-5.6`
  - 스크린샷, DOM, 브라우저 콘솔, 플로우 evidence 점검처럼 시각 판단 비용이 큰 작업에 사용한다.
- 프롬프트/구조 감사 정책: `gpt-5.6`, high reasoning
...
- 이미지 작업 정책: 최신 상위 모델 우선
  - 이미지 생성 초안, 이미지 해석, 시각 회귀 판단, 디자인 방향 비교처럼 이미지가 핵심 입력 또는 출력인 작업은 현재 최신 모델인 `gpt-5.6`을 우선 사용한다.
...
- 생성 서비스 레포는 새 화면, 랜딩, 대시보드 같은 first-pass UI 작업을 위한 `docs/prompting/ui-foundation-prompt-template.kr.md` 기본 템플릿을 포함한다.
...
- 프롬프트/구조 감사는 `gpt-5.6`로 수행하고, 결과는 문서와 검증 스크립트에 반영한다.
