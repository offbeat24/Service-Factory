...
- 기본 루트 작업은 `gpt-5.5`로 시작한다.
- `gpt-5.6`은 초기 제품 방향, 아키텍처/데이터 경계, 인증/결제/보안, 복잡한 UX 계층, 큰 작업 분해, 반복 실패 후 복구 계획, 계약/템플릿 정책 변경, 프롬프트/구조 감사, 최종 리뷰에 승격한다.
- 프롬프트/구조 감사는 `AGENTS.md`, `.codex/`, 계약, 템플릿, 스크립트, 정책 문서를 함께 판단하는 작업으로 보고 `gpt-5.6`을 강제한다.
- `gpt-5.4`는 최하위 대화형 모델로 두고, 반복 구현, 문서와 evidence 정리, 긴 감사와 마이그레이션 같은 저위험 작업에 배정한다.
- 이미지 생성 초안, 이미지 해석, 스크린샷 기반 시각 판단은 최신 상위 모델로 처리한다. 현재 기본 핀은 `gpt-5.6`이다.
